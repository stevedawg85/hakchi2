...
namespace com.clusterrr.hakchi_gui
{
    public partial class WorkerForm : Form
    {
        ...
        public void DoPartitionDump(Tasks task)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        case Tasks.DumpNandB:
                            partitionSize = int.Parse(sshClientWrapper.ExecuteSimple("echo $((($(hexdump -e '1/4 \"%u\"' -s $((0x28)) -n 4 /dev/mapper/root-crypt)+0xfff)/0x1000))").Trim()) * 4;
                            break;
                        case Tasks.DumpNandC:
                        case Tasks.FlashNandC:
                ...
                if (task != Tasks.FlashNandC)
                {
                    ...
                    using (var file = new TrackableFileStream(NandDump, FileMode.Create))
                    {
                        ...
                        {
                            case Tasks.DumpNandB:
                                sshClientWrapper.Execute($"dd if=/dev/mapper/root-crypt bs=4K count={partitionSize / 4}", null, file);
                                break;
                            case Tasks.DumpNandC:
        ...
        public static void GetMemoryStats()
        {
            ...
            {
                var sshClientWrapper = MainForm.sshClientWrapper;
                var storage = sshClientWrapper.ExecuteSimple("df \"$(hakchi findGameSyncStorage)\" | tail -n 1 | awk '{ print $2 \" | \" $3 \" | \" $4 }'", 2000, true).Split('|');
                ExternalSaves = sshClientWrapper.ExecuteSimple("mount | grep /var/lib/clover").Trim().Length > 0;
                WrittenGamesSize = long.Parse(sshClientWrapper.ExecuteSimple("du -s \"$(hakchi findGameSyncStorage)\" | awk '{ print $1 }'", 2000, true)) * 1024;
                SaveStatesSize = long.Parse(sshClientWrapper.ExecuteSimple("du -s \"$(readlink /var/saves)\" | awk '{ print $1 }'", 2000, true)) * 1024;
        ...
        {
            var sshClientWrapper = MainForm.sshClientWrapper;
            var customFirmwareLoaded = sshClientWrapper.ExecuteSimple("hakchi currentFirmware") != "_nand_";
            string board, region;
            if (!customFirmwareLoaded || !realType)
            {
                board = sshClientWrapper.ExecuteSimple("cat /etc/clover/boardtype", 3000, true);
                region = sshClientWrapper.ExecuteSimple("cat /etc/clover/REGION", 3000, true);
            }
            else
            {
                sshClientWrapper.ExecuteSimple("cryptsetup open /dev/nandb root-crypt --readonly --type plain --cipher aes-xts-plain --key-file /etc/key-file", 3000);
                sshClientWrapper.ExecuteSimple("mkdir -p /var/squashfs-original", 3000, true);
                sshClientWrapper.ExecuteSimple("mount /dev/mapper/root-crypt /var/squashfs-original", 3000, true);
                board = sshClientWrapper.ExecuteSimple("cat /var/squashfs-original/etc/clover/boardtype", 3000, true);
                region = sshClientWrapper.ExecuteSimple("cat /var/squashfs-original/etc/clover/REGION", 3000, true);
                sshClientWrapper.ExecuteSimple("umount /var/squashfs-original", 3000, true);
                sshClientWrapper.ExecuteSimple("rm -rf /var/squashfs-original", 3000, true);
                sshClientWrapper.ExecuteSimple("cryptsetup close root-crypt", 3000, true);
            }
        ...
        public static void ShowSplashScreen()
        {
            ...
            sshClientWrapper.ExecuteSimple("uistop");
        ...
        public void UploadGames()
        {
            ...
            try
            {
                ...
                {
                    SetStatus(Resources.CleaningUp);
                    sshClientWrapper.ExecuteSimple("find \"$(hakchi findGameSyncStorage)/\" -maxdepth 1 | grep -vEe '(/snes(-usa|-eur|-jpn)?|/nes(-usa|-jpn)?|/)$' | while read f; do rm -rf \"$f\"; done", 0, true);
                }
                progress += 5;
                SetProgress(progress, maxProgress);
                ...
                string gamesOnDevice = sshClientWrapper.ExecuteSimple($"mkdir -p \"{gameSyncPath}\"; cd \"{gameSyncPath}\"; find . -type f -exec sh -c \"stat \\\"{{}}\\\" -c \\\"%n %s %y\\\"\" \\;", 0, true);
                ...
                SetStatus(Resources.CleaningUp);
                sshClientWrapper.ExecuteSimple($"for f in $(find \"{gameSyncPath}\" -type d -mindepth 1 -maxdepth 2); do {{ ls -1 \"$f\" | grep -v pixelart | grep -v autoplay " +
        ...
        private void DeleteRemoteApplicationFiles(IEnumerable<ApplicationFileInfo> filesToDelete)
        {
            ...
            try
            {
                sshClientWrapper.Execute("cat > /tmp/cleanup.sh", commandBuilder, null, null, 5000, true);
                sshClientWrapper.ExecuteSimple("chmod +x /tmp/cleanup.sh && /tmp/cleanup.sh", 0, true);
            }
            finally
            {
                sshClientWrapper.ExecuteSimple("rm /tmp/cleanup.sh");
            }
        }
        ...
        public static void SyncConfig(Dictionary<string, string> Config, bool reboot = false)
        {
            ...
            sshClientWrapper.Execute($"cat >> {configPath}", config, null, null, 3000, true);
            config.Dispose();
            if (reboot)
            {
                try
        ...
        public static Image TakeScreenshot()
        {
            ...
            sshClientWrapper.ExecuteSimple("hakchi uipause");
            sshClientWrapper.Execute("cat /dev/fb0", null, rawStream, null, 1000, true);
            sshClientWrapper.ExecuteSimple("hakchi uiresume");
        ...
        public void ProcessMods()
        {
            if (MainForm.IsConsoleConnected)
            {
                MainForm.sshClientWrapper.ExecuteSimple("uistop");
                ...
                {
                    MainForm.sshClientWrapper.ExecuteSimple("hakchi packs_install /tmp/hmods/", 0);
        ...
        void UpdateLocalCache()
        {
            ...
            var sshClientWrapper = MainForm.sshClientWrapper;
            if (!MainForm.IsConsoleConnected) return;
