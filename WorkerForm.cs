...
namespace com.clusterrr.hakchi_gui
{
    public partial class WorkerForm : Form
    {
        ...
        public class NandStat
        {
            ...
            {
                var clovershell = MainForm.Clovershell;
                var nand = clovershell.ExecuteSimple("df | grep " + device + " | tail -n 1 | awk '{ print $2 \" | \" $3 \" | \" $4 }'", 30000, true).Split('|');
                if (nand.Length >= 3)
                {
        ...
        public static void GetMemoryStats()
        {
            ...
            if (extraFs)
            {
                WritedGamesSize += int.Parse(clovershell.ExecuteSimple(String.Format("mkdir -p /var/lib/hakchi/extrafs/games/{0}_{1}/ && du -s /var/lib/hakchi/extrafs/games/{0}_{1}", MainForm.board, MainForm.region) + " | awk '{ print $1 }'", 1000, true)) * 1024;
            }
        ...
        public void UploadGames()
        {
            ...
            string extraGamesPath = String.Format("{0}/games/{1}_{2}", extraFsPath, MainForm.board, MainForm.region);
            int progress = 0;
            int maxProgress = 400;
                    ...
                    using (var gamesTar = new TarStream(Path.Combine(tempGamesDirectory, gameFolder.path)))
                    {
                        ...
                        if (gamesTar.Length > 0)
                        {
                            ...
                            if (extraFs && extraFsStats.freeBytes - 102400 > gameFolder.size)
                            {
                                ...
                                clovershell.Execute(string.Format("tar -xvC {0}/{1} && sync", extraGamesPath, gameFolder.path), gamesTar, null, null, 30000, true);
                            ...
                            {
                                clovershell.Execute(string.Format("tar -xvC {0}{1}/{2} && sync", rootFsPath, gamesPath, gameFolder.path), gamesTar, null, null, 30000, true);
                            }
